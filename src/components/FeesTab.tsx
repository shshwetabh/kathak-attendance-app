...
import { CheckCircle, Clock, AlertTriangle, Send, X, Check, Plus, Search, Filter, IndianRupee } from 'lucide-react';
...
export const FeesTab: React.FC<FeesTabProps> = ({ batches, students }) => {
  ...
  const openGeneralPaymentModal = () => {
    ...
    if (firstStudent) {
      openPaymentModalForStudent(firstStudent);
    } else {
  ...
  const handleStudentSelectionChange = (stuId: string) => {
    setSelectedStudentId(stuId);
    const stu = students.find((s) => s.id === stuId);
    if (stu) {
      setActiveModalStudent(stu);
      const batch = getStudentBatch(stu.batch_id);
      const existing = payments[stu.id];
      setAmountPaid(existing ? existing.amount_paid : batch ? batch.monthly_fee : 2500);
    }
  };
  ...
  const handleSavePayment = async (e: React.FormEvent) => {
    e.preventDefault();
    const targetStudent = activeModalStudent || students.find((s) => s.id === selectedStudentId);
    if (!targetStudent) return;

    const batch = getStudentBatch(targetStudent.batch_id);
    ...
    <div className="space-y-4 pb-24 max-w-md mx-auto px-4 pt-3">
      ...
      <div className="space-y-2.5">
        <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wider px-1">
          Student Payment Directory ({filteredStudents.length})
        </h2>
        ...
        {filteredStudents.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 text-center text-slate-500 border border-slate-200 space-y-2">
            ...
            <p className="text-xs text-slate-500">
              Add your students in the <strong>Students</strong> tab to see their monthly fee records here.
            </p>
          </div>
        ) : (
          ...
          <div className="bg-white rounded-3xl p-5 w-full max-w-sm shadow-xl space-y-4 animate-scale-up">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">{selectedMonth}</p>
              </div>
              <button
